...
class Result {
    ...
    public static String dayOfProgrammer(int year) {
        ...
        boolean isValid = (year >=1700 && year<=2700);


        //1700 den 1917: dung lich Julian
        //19918: Sau ngay 31st January  la 14th February
        //1919: Dung lich Gregorian

        boolean isJulian = (year>=1700 &&year<=1917)?true:false;
        System.out.println("isJulian " + isJulian);
        boolean isLeapYear = (isJulian && (year%4==0))?true:(!isJulian&&((year%400==0 ||(year%4 ==0 && year%100!=0)))?true:false);
        ...
        while (dayth>0){
            ...
            switch (month){
                ...
                default:
                    day=dayth;
                    dayth=dayth-(isLeapYear?29:28);
                    break;
            }
        }
        String monthdd = "0" + month;
        String daydd = "0" + day;
        System.out.println();
        System.out.println(daydd);

        return year+"."+monthdd.substring(monthdd.length() -2, monthdd.length())+"."+daydd.substring(daydd.length() -2, daydd.length());
    }
